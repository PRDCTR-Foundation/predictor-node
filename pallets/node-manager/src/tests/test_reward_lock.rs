...
fn seed_locked(owner: &AccountId, amount: u128) {
    ...
    Balances::make_free_balance_be(&pot, amount + PRD);
}
...
fn withdraw_full_at_week_one_forfeits_52_percent() {
    ...
    ext.execute_with(|| {
        ...
        // treasury source.
        assert_eq!(Balances::free_balance(treasury_account()) - treasury_before, 52 * PRD,);
        assert_eq!(LockedRewards::<TestRuntime>::get(owner), 0);
...
fn withdraw_partial_with_limit_applies_penalty_to_slice() {
    ...
    ext.execute_with(|| {
        ...
        assert_ok!(NodeManager::withdraw_rewards(RawOrigin::Signed(owner).into(), Some(40 * PRD),));

        // forfeit = 26% of 40 = 10.4; net = 29.6.
        assert_eq!(Balances::free_balance(owner), 29 * PRD + 6 * PRD / 10);
        assert_eq!(LockedRewards::<TestRuntime>::get(owner), 60 * PRD);
        assert_eq!(TotalLockedRewards::<TestRuntime>::get(), 60 * PRD);

        // The remainder withdraws at the same rate.
        assert_ok!(NodeManager::withdraw_rewards(RawOrigin::Signed(owner).into(), None));
        assert_eq!(
            Balances::free_balance(owner),
            (29 * PRD + 6 * PRD / 10) + (60 * PRD - 60 * PRD * 26 / 100),
        );
        assert_eq!(TotalLockedRewards::<TestRuntime>::get(), 0);
    });
}
...
fn withdraw_routes_forfeit_to_configured_destination() {
    ...
    ext.execute_with(|| {
        ...
        assert_eq!(Balances::free_balance(destination), 52 * PRD);
...
fn withdraw_full_succeeds_when_pot_holds_exactly_the_locked_amount() {
    ...
    ext.execute_with(|| {
        ...
        assert_eq!(Balances::free_balance(owner), 48 * PRD, "net reaches the owner");
        assert_eq!(Balances::free_balance(destination), 52 * PRD, "forfeit reaches destination");
        assert_eq!(Balances::free_balance(pot), 0, "pot drained to zero");
        assert_eq!(LockedRewards::<TestRuntime>::get(owner), 0);
        assert_eq!(TotalLockedRewards::<TestRuntime>::get(), 0);
    });
...
fn withdraw_error_paths() {
    ...
    ext.execute_with(|| {
        ...
        assert_noop!(
            NodeManager::withdraw_rewards(RawOrigin::Signed(owner).into(), Some(101 * PRD)),
