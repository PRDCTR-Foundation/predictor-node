...
mod pallet {
    ...
    use frame_support::{
        ...
        traits::{IsSubType, IsType, StorageVersion},
        PalletId,
    };
    use frame_system::{
        ensure_signed,
    ...
    pub use sp_runtime::{
        traits::{Dispatchable, Get, IdentifyAccount, Verify, Zero},
        ...
        T: Config,
    {
        ...
        HybridRouterExecuted {
            ...
            /// by the trader from AMM and order-book.
    ...
    pub enum Error<T> {
        ...
        SenderIsNotSigner,
        /// The transaction type is not supported for signed transactions
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// * `strategy`: The strategy to handle the remaining order when the `max_price` is
        ///   reached.
        ///
        /// The elements of `orders` are the orders that the router may use to execute the order. If
        /// any of these orders are already filled, they are ignored. It is not necessary
        /// for the router to use all specified orders. The smaller the vector, the larger
        /// the risk that the AMM is used to fill large chunks of the order.
        ///
        /// The `orders` vector **must** be sorted in ascending order by the price of their
        /// associated orders. Failing this, the behavior of `buy` is undefined.
        ///
        /// If the maximum price is reached before the entire buy order is filled, the `strategy`
        /// parameter decides if the order is rolled back (`Strategy::ImmediateOrCancel`) or
        /// if a limit order for the remaining amount is placed (`Strategy::LimitOrder`).
        ///
        /// Complexity: `O(n)`
        #[pallet::call_index(0)]
        ...
        /// * `orders`: A list of orders from the book to use.
        /// * `strategy`: The strategy to handle the remaining order when the `min_price` is
        ///   reached.
        ///
        /// The elements of `orders` are the orders that the router may use to execute the order. If
        /// any of these orders are already filled, they are ignored. It is not necessary
        ...
        ///
        /// The `orders` vector **must** be sorted in ascending order by the price of their
        /// associated orders. Failing this, the behavior of `sell` is undefined.
        ///
        /// If the maximum price is reached before the entire buy order is filled, the `strategy`
        /// parameter decides if the order is rolled back (`Strategy::ImmediateOrCancel`) or
        /// if a limit order for the remaining amount is placed (`Strategy::LimitOrder`).
        ...
        // Signed buy extrinsic
        #[pallet::call_index(2)]
        #[pallet::weight(T::WeightInfo::signed_buy(*asset_count as u32, orders.len().saturated_into::<u32>()))]
        #[frame_support::transactional]
        #[allow(clippy::too_many_arguments)]
        pub fn signed_buy(
            origin: OriginFor<T>,
        ...
        // Signed sell extrinsic
        #[pallet::call_index(3)]
        #[pallet::weight(T::WeightInfo::signed_sell(*asset_count as u32, orders.len().saturated_into::<u32>()))]
        #[frame_support::transactional]
        pub fn signed_sell(
            origin: OriginFor<T>,
            proof: Proof<T::Signature, T::AccountId>,
        ...
        T: Config,
    {
        ...
        fn maybe_fill_orders(
            ...
            for &order_id in orders {
                ...
                match tx_type {
                    TxType::Buy => {
                        // existing order is willing to give the required `asset` as the
                        // `maker_asset`
                        ensure!(
                    ...
                    TxType::Sell => {
                        // existing order is willing to receive the required `asset` as the
                        // `taker_asset`
                        ensure!(
        ...
        /// # Arguments
        ///
        /// * `strategy` - The strategy to handle the remaining non-zero amount when the `max_price`
        ///   is reached.
        /// * `who` - The account ID of the user performing the transaction.
        /// * `market_id` - The ID of the market.
        ...
        /// Executes a trade by routing the order to the Automated Market Maker (AMM) and the Order
        /// Book to achieve the best average execution price.
        ///
        /// # Arguments
        ///
        /// * `tx_type` - The type of transaction (Buy or Sell).
        ...
        pub(crate) fn do_trade(
            ...
            strategy: Strategy,
                ...
                price_limit <= PredictionMarketBase::<BalanceOf<T>>::get()?,
    ...
    // Updated InnerCallValidator implementation
